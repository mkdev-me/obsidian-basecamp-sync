...
In shared-service mode, the service processes OAuth codes and tokens during login and refresh. Note and attachment requests go directly from Obsidian to Basecamp. The mkdev service is hosted on Cloudflare and its implementation is maintained in a private infrastructure repository, separate from this MIT-licensed plugin. It is stateless: encrypted, expiring login state and code tickets travel with the requests, and tokens are returned directly to the device. It does not keep a login/token database or log credentials. The service operator is responsible for hosting-layer log retention and access controls. The **My own integration** mode exchanges and refreshes credentials directly with Basecamp and does not use mkdev's service.
